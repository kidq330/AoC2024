...
let given_tests =
  ...
  ; ( "list 4"
    >:: fun _ -> [1; 3; 2; 4; 5] |> BatList.singleton |> solve |> assert_equal 0
    )
  ; ( "list 5"
    >:: fun _ -> [8; 6; 4; 4; 1] |> BatList.singleton |> solve |> assert_equal 0
    )
  ; ( "list 6"
...
let qtest_counterexample_1 =
  ...
  ; ( "qtest_counterexample_1:solve List.fold_right check"
    >:: fun _ ->
    assert_equal false (List.fold_right ( || ) [false; false; false] false) )
  ; ( "qtest_counterexample_1:solve List.fold_left check"
    >:: fun _ ->
